        ...
        <div class="flex ml-2">
         <button id="dropdownNotificationButton" data-dropdown-toggle="dropdownNotification" class="inline-flex items-center text-sm font-medium text-center text-gray-500 hover:text-gray-900 focus:outline-none" type="button"> 
            ...
            <div id="dropdownNotification" class="hidden z-20 w-full rounded-b-md max-w-sm bg-white divide-y divide-gray-100 shadow" aria-labelledby="dropdownNotificationButton">
                  ...
                  <div v-for="notif in notificationData" :key="notif" class="divide-y-2 divide-gray-100">
...
export default {
    ...
    methods: {
        init() {
         axios.get(this.$store.state.serverUrl + '/notifications/findAllNotficationForOffice/' + this.$store.state.user.userDetails.office.id, {headers: {Authorization: `Bearer  ${this.token}`}}).then((res)=>{
