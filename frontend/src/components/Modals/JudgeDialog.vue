<template>
    <div>
        <div class="modal fade fixed top-0 left-0 right-0 bottom-0 overflow-x-hidden hidden w-full h-full outline-none" id="staticBackdropJudges" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div  class="modal-dialog relative top-16 max-w-[30rem] pointer-events-none">
                <div class="modal-content border-none shadow-lg relative flex flex-col max-w-5x1 pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                    <div class="modal-header flex flex-shrink-0 items-center justify-between bg-white p-4 border-b border-gray-200 rounded-t-md">
                         <div class="text-sm font-semibold leading-normal text-gray-800" id="exampleModalLabel">
                          ...
                          <button type="button" class="p-2" id="close-btn3" data-bs-dismiss="modal"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="gray" class="w-6 h-6">
                ...
                <div class="modal-body rounded-md relative">
                    ...
                    <div class="text-left ml-6 mt-3.5 text-[13px] text-gray-800 font-bold">Name</div>
                    <div><input 
                        v-model="name" 
...
export default {
    ...
    methods: {
        ...
        initializeUpdate(data){
            ...
            this.judgeId = data.id
            this.name = data.name
            this.status = data.status
        },
        checkForm(){
            this.isSubmitting = true
            const formValid = [this.name]
            .map((x) => x.trim())
            .every(Boolean);
            ...
            if(formValid){
                ...
                if(this.action == "add"){
                    axios.post(this.$store.state.serverUrl + '/judges', formData, {headers: {Authorization: `Bearer  ${this.token}`}}).then((res)=>{
                        if(res){
                            this.$emit('refresh')
                            document.getElementById('close-btn3').click();

                            const toast = useToast();
                            toast.success("Document Type Succesfully Saved", {
                            timeout: 2000,
                            closeOnClick: true,
                            pauseOnHover: false,
                    ...
                    axios.post(this.$store.state.serverUrl + '/judges/update', formData, {headers: {Authorization: `Bearer  ${this.token}`}}).then((res)=>{
                        if(res){
                            this.$emit('refresh')
                            document.getElementById('close-btn3').click();

                            const toast = useToast();
                            toast.success("Document Type Succesfully Updated", {
                            timeout: 2000,
                            closeOnClick: true,
                            pauseOnHover: false,
                            showCloseButtonOnHover: true,
