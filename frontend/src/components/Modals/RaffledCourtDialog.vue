<template>
    <div>
        <div class="modal fade fixed top-0 left-0 right-0 bottom-0 overflow-x-hidden hidden w-full h-full outline-none" id="staticBackdropRaffledCourt" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
...
export default {
    ...
    methods: {
        ...
        initializeAdd(){
            ...
            this.action = "add"
            this.title = "Add New Raffled Court"
            this.raffledCourtId = null
            this.description = "",
            this.status = true
        },
        initializeUpdate(data){
            this.isSubmitting = false
            this.action = "update"
            this.title = "Update Raffled Court"
            this.raffledCourtId = data.id
        ...
        checkForm(){
            ...
            if(formValid){
                let formData = {
                    raffledCourtId: this.raffledCourtId,
                    description: this.description,
                    status: this.status
                } 
                if(this.action == "add"){
                    axios.post(this.$store.state.serverUrl + '/raffled-court', formData, {headers: {Authorization: `Bearer  ${this.token}`}}).then((res)=>{
                        if(res){
                            this.$emit('refresh')
                            document.getElementById('close-btn4').click();

                            const toast = useToast();
                            toast.success("Raffled Court Succesfully Saved", {
                            timeout: 2000,
                            closeOnClick: true,
                            pauseOnHover: false,
                    ...
                    axios.post(this.$store.state.serverUrl + '/raffled-court/update', formData, {headers: {Authorization: `Bearer  ${this.token}`}}).then((res)=>{
                        if(res){
                            this.$emit('refresh')
                            document.getElementById('close-btn4').click();

                            const toast = useToast();
                            toast.success("Raffled Court Succesfully Updated", {
                            timeout: 2000,
                            closeOnClick: true,
                            pauseOnHover: false,
                            showCloseButtonOnHover: true,
