      ...
      <div class="bg-white py-8 px-2 shadow sm:rounded-lg sm:px-10">
        ...
        <form class="space-y-6" @submit.prevent>
            <div class="relative mt-6">
              <input @keyup.enter="userLogin()" v-model="user.email" :class="{ invalid: isSubmitting && !user.email.trim() }" type="email" id="floating_outlined" class="block px-2.5 pb-2.5 pl-5 pt-3 w-full text-xs text-gray-900 bg-transparent rounded-3xl border-1 border-gray-400 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-purple-600 peer" placeholder=" " />
              <label for="floating_outlined" class="absolute text-[13px] text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 ml-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 focus:border-purple-600 peer-focus:text-purple-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1">Email</label>
            </div>

            <div class="relative">
              <input @keyup.enter="userLogin()" v-model="user.password" :class="{ invalid: isSubmitting && !user.password.trim() }" :type="show ? 'password': 'text'" id="floating_outlined2" class="block px-2.5 pb-2.5 pl-5 pt-3 w-full text-xs text-gray-900 bg-transparent rounded-3xl border-1 border-gray-400 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-purple-600 peer" placeholder=" " />
              <label for="floating_outlined2" class="absolute text-[13px] text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 ml-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 focus:border-purple-600 peer-focus:text-purple-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1">Password</label>
