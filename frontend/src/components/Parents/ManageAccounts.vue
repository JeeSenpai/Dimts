<template>
    <div class="checkFade animated">
        ...
        <div class="flex flex-row justify-between">
            ...
            <div>
                <button @click="showAddDialog()" class="mr-3 rounded-xl px-2 mb-2 py-1.5 text-sm border-[1.5px] font-semibold bg-[#BF40BF] border-[#BF40BF] text-white" type="button" data-bs-toggle="modal" data-bs-target="#staticBackdropAccount">
                ...
                <tbody v-if="displayedPaginatedData.length > 0">
                    <tr v-for="data in displayedPaginatedData" :key="data" class="bg-white border-b text-xs text-center">
                        <td class="px-5 py-3 text-black text-left">
                            {{ data.userDetails.lname }}, {{ data.userDetails.fname }} {{ !data.userDetails.mname ? '': data.userDetails.mname[0] + '.' }}
                        </td>
                        <td class="px-5 py-3 ">
                            {{ data.email }}
                        </td>
                        <td class="px-5 py-3">
                            {{ data.userDetails.userType.description }}
                        </td>
                        <td class="px-5 py-3 ">
                           {{ data.userDetails.office.description }}
                        </td>
                        ...
                        <td class="py-3 px-2 text-right">
                            <button @click="showUpdateDialog(data)" type="button" data-bs-toggle="modal" data-bs-target="#staticBackdropAccount" class="bg-transparent mr-4 py-1.5">
...
import CitizenAccount from '../Utils/CitizenAccount.vue'
import ManageAccountDialog from '../Modals/ManageAccountDialog.vue'
...
export default {
    ...
    methods: {
        init(){
            axios.get(this.$store.state.serverUrl + '/users', {headers: {Authorization: `Bearer  ${this.token}`}}).then((res)=>{
                this.data = res.data
        ...
        showAddDialog(){
            this.$refs.ManageAccountDialog.initializeAdd()
        },
        showUpdateDialog(data){
            this.$refs.ManageAccountDialog.initializeUpdate(data)
        },
